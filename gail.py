...
from architecture import ContGaussianPolicy, ContTwinQNet, Discriminator
from utils import gen_noise, polyak_update, TensorWriter
...
class GailContSAC_SRC:
    def __init__(self, policy_config, value_config, env,target_env, device, expert, expert_model, log_dir="latest_runs",
        ...
        self.ilmemory = ReplayBuffer(1000000, self.batch_size)
    ...
    def train(self, num_games, deterministic=False):
        ...
        for i in range(num_games):
            ...
            if i%100==0:
                new_src, new_tgt = self.eval_src(10), self.eval(10)
                src.append(new_src)
                tgt.append(new_tgt)
                print('source reward: {}, target reward: {}'.format(new_src, new_tgt))
    ...
    def expert_reward(self,state,action, next_state):
        ...
        with torch.no_grad():
            ...
            return -math.log(self.discrim_net(state_action)[0].item()) #* wt.item()
